...
For the purpose of this walkthrough, we will reused the completed pipeline from the tutorial [Your pipeline from A to Z](/pipeline/2-tutorial_explore).

:::caution[If you did not complete the pipeline creation tutorial]
Don't worry! You can still catch up by following the [setup section](/pipeline/1-setup).
...
This tutorial takes into account you have QC files published by your module,
using the code snippet availabe in the [module contribution tutorial](/contribute/create-your-module/7-quality-control).
...
If you look at the outputs of `RECONST_DTIMETRICS` (either in the [API documentation](/api/modules/reconst/dtimetrics)
...
up reports. Finally, as we saw in the [MultiQC inputs](#1-modify-the-multiqc-inputs) section,
...
the title of our section containing our FA QC image and its description. 
    ...
    repository](https://github.com/nf-neuro/modules/blob/main/assets/multiqc_config.yml).
    :::
