...
In **nf-neuro**, we use the **<code class="dark:text-slate-200 text-slate-700">task.ext</code> parameter namespace** to define
configuration options that user can use to fine-tune the behavior of a module. Those options have two
specificities :

<Steps>
1. **They are _optional_ by default**. If a `parameter` is not defined, <code class="dark:text-slate-200 text-slate-700">task.ext.parameter</code>
   will return the value <code class="dark:text-blue-300 text-blue-500">null</code>.
2. **They are _static_**. Their value is bound to the module's name. _Dynamic parameters_ have to be defined using
   [inputs](/contribute/create-your-module/2-main#the-input-section).
</Steps>
...
The **elvis operator** (<code class="dark:text-orange-300 text-yellow-800">?:</code>) will select <code class="dark:text-orange-300 text-yellow-800">task.ext.number_of_coils</code> if it is defined,
and the _default value_ <code class="dark:text-orange-300 text-yellow-800">1</code> otherwise.
...
<Steps>
1. As stated, we define a new **optional input file**, <code class="dark:text-slate-200 text-slate-700">path(noise_mask)</code>, as well as an
...
those parameters in the [subworkflow configuration](/contribute/create-your-subworkflow/4-configuration) section. For now,
