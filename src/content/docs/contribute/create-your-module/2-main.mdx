...
1. The `args` variable is a **string** of custom arguments to pass to a command line inside the
  ...
  **In nf-neuro, the `args` variable is not used directly. Instead, you define [task.ext](/contribute/create-your-module/3-configuration)
...
```groovy {"1": 2-3} {"2": 4-5} {"3": 8} {"4": 10} {"5": 11} {"6": 11} {"7": 12}
script:
    def prefix = task.ext.prefix ?: "${meta.id}"
    def args = task.ext.args ?: []
    def nthreads = task.ext.single_thread ? 1 : task.cpus
    args += "--processes $nthreads"
...
2. In the **groovy header**, we also added a variable named `nthreads`. This Groovy variable uses the <InlineCode parts={frontmatter.taskDotCpu} /> and <code class="dark:text-lime-300 text-sky-700">task.ext.single_thread</code>
  variables to limit the command's thread count to the number of CPUs allocated to the process.
3. The scilpy command can handle its own parallelization. Using <code class="dark:text-orange-300 text-yellow-800">export</code>,
   we declared **1 environment variable** to limit the threads used by dependencies (e.g. `OMP`).
   So far, we found 3 recurring cases depending on the tool your module used:
   - if your module uses MRTRIX:
    ```
    export OMP_NUM_THREADS=${task.ext.single_thread ? 1 : task.cpus}
    export MRTRIX_RNG_SEED=${task.ext.mrtrix_rng_seed ? task.ext.mrtrix_rng_seed : "1234"}
    ```
   - if your module uses SCILPY:
    ```
    export OMP_NUM_THREADS=${task.ext.single_thread ? 1 : task.cpus}
    ```
   - if your module uses ANTS:
    ```
    ...
    ```
4. The <code class="dark:text-orange-300 text-yellow-800">scil_denoising_nlmeans.py</code> command is called on an input
   <code class="dark:text-lime-300 text-sky-700">$image</code>, a variable you will define in the `input` section later.
5. The denoised image is written to a file named <InlineCode parts={frontmatter.coloredDenoised} />, with
...
6. The user's custom arguments are passed to the command, by unraveling the <code class="dark:text-orange-300 text-yellow-800">args</code> variable
   using <code class="dark:text-orange-300 text-yellow-800">join</code>, a **groovy** operator.
7. The groovy var <code class="dark:text-orange-300 text-yellow-800">args</code> is used here to compile the different params.
</Steps>
:::
...
```diff lang="groovy" ins=", path(mask) /* optional, input = [] */" {"1": 2} {"2": 8} {"3": 15}
input:
...
**We gathered versioning commands for common neuroimaging software. [Take a look here](/guides/how-to/versioning/1-common-software-version).**
:::
